...
function _M:purge(shadow)
  ...
  if shadow and #self.mlcaches == 2 then
    local _
    _, _, node_id = self.mlcaches[current_page]:peek("kong:node_id")
  ...
  if node_id then
    log(DEBUG, "setting node id")
    local ok, err = self:safe_set("kong:node_id", node_id, shadow)
    if not ok then
      log(ERR, "failed to set node id: ", err)
    end
  end
end
