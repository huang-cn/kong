...
do
  ...
  --
  -- This method can be used in both `http` and `stream` subsystem. However,
  -- the information returned may be slightly different.
  --
  ...
  --
  -- Notes for stream proxy mode, this function's returns are mostly the same, but
  -- the `request` field is renamed to `session` to better reflect the connection
  -- type.
  --
